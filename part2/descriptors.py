...
# descriptors.py output:  qm9_descriptors_p2.csv  [smiles, MolWt, LogP, ... gap]
...
def get_descriptors(smiles):
    ...
    return {
        ...
        'FractionCSP3': rdMolDescriptors.CalcFractionCSP3(mol),                         # (num sp3 C atoms) / (total num C atoms)
                                                                                        # expected to be top SHAP feature
        ...
        'MaxPartialCharge': Descriptors.MaxPartialCharge(mol),                          # max Gasteiger charge on any atom
        'MinPartialCharge': Descriptors.MinPartialCharge(mol),                          # min Gasteiger charge

        # aromatic
        'NumAromaticRings': Descriptors.NumAromaticRings(mol),                          # 'c1ccccc1' (double bonds)
        'NumAromaticCarbocycles': rdMolDescriptors.CalcNumAromaticCarbocycles(mol),     # 'c1ccccc1' (double bonds, all C atoms)
        'NumAromaticHeterocycles': rdMolDescriptors.CalcNumAromaticHeterocycles(mol),   # aromatic rings with at least one non-C atom
        'NumAromaticAtoms': sum(1 for a in mol.GetAtoms() if a.GetIsAromatic()),
        'FractionAromaticAtoms': sum(1 for a in mol.GetAtoms() if a.GetIsAromatic()) / mol.GetNumHeavyAtoms(),

        # other ring descriptors
        ...
        'NumRings': rdMolDescriptors.CalcNumRings(mol),                                 # total rings including fused
        'NumBridgeheadAtoms': rdMolDescriptors.CalcNumBridgeheadAtoms(mol),             # atoms shared between rings
...
raw = pd.read_csv(CSV_DIR/'qm9_aromatic.csv')
...
print(desc_df.shape)
# (21981, 25) - smiles + 23 descriptors + gap
print(desc_df.head())

desc_df.to_csv(CSV_DIR/'qm9_descriptors_p2.csv', index=False)